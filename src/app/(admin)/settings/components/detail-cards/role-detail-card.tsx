...
import { Shield, Info, Calendar, Users, KeyRound, CheckCircle2, X, FileText, MessageSquare, StickyNote, BookOpen, Clock, Trash2, Pencil } from 'lucide-react';
...
export function RoleDetailsCard({ role, onClose, onEdit, onEditPermissions }: RoleDetailsCardProps) {
    return (
        <div className="w-[340px] bg-white dark:bg-muted rounded-xl shadow-sm p-4 ml-6 flex flex-col border border-gray-100 dark:border-gray-800">
            ...
            <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                    <div className="font-semibold flex items-center gap-1.5" style={{ lineHeight: '1.2' }}>
                        <span className={`${role.name.length > 22 ? 'text-sm' : 'text-base'} truncate`}>
                            {role.name}
                        </span>
                ...
                <button onClick={onClose} className="ml-2 p-1.5 rounded-full hover:bg-muted flex-shrink-0">
                    <X className="h-4 w-4" />
                </button>
            </div>
            ...
            <div className="mb-4">
                ...
                <div className="space-y-1.5 max-h-[120px] overflow-y-auto pr-1">
                    {role.permissions.map((permission, index) => (
                        <div key={permission.id} className="text-xs flex items-start gap-1.5">
